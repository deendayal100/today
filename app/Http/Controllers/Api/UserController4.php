...
class UserController extends Controller
{
    ...
    public function add_player_ingroup(Request $request){
        ...
        if($validator->fails()){
            return response()->json(['status'=>201,'message'=>$validator->errors()->first('player_id'),'group_unique_id'=>null]);
        }
        $user_res = User::where(['id'=>$request->player_id,'status'=>1])->first();
        if($user_res == '' && $user_res == null){
            return response()->json(['status'=>201,'message'=>"Player Not Existed or Not Active.",'group_unique_id'=>null]);
        }
        $validator = Validator::make(['ante_amount_id'=>$request->ante_amount_id],[
            'ante_amount_id' => 'required|numeric'
        ]);
        if($validator->fails()){
            return response()->json(['status'=>201,'message'=>$validator->errors()->first('ante_amount_id'),'group_unique_id'=>null]);
        }
        $validator = Validator::make(['player_no_id'=>$request->player_no_id],[
            'player_no_id' => 'required|numeric'
        ]);
        if($validator->fails()){
            return response()->json(['status'=>201,'message'=>$validator->errors()->first('player_no_id'),'group_unique_id'=>null]);
        }
        ...
        if(count($group_res)>0){
            ...
            if(count($group_res1)>0){
                return response()->json(['status'=>202,'message'=>"Player Already Added in This Group.",'group_unique_id'=>$group_res1[0]->group_unique_id]);  
            }else{
                $ante_amnt=$bonus_amnt=0;
                ...
                if($tl_players != '' && $tl_players != null){
                    if(count($group_res)+1 < $tl_players->player_number){
                        ...
                        $user->group_unique_id=$group_res[0]->group_unique_id;
                        $res= $user->save();
                        if($res > 0){
                            $ante_amnt = AnteAmount::where('id',$request->ante_amount_id)->value('ante_amount');
                            $bonus_amnt =  BonusPot::where('group_unique_id',$group_res[0]->group_unique_id)->value('amount');
                            BonusPot::where('group_unique_id',$group_res[0]->group_unique_id)->update(['amount'=>($ante_amnt + $bonus_amnt)]);
                            return response()->json(['status'=>200,'message'=>"Player Added in Group SuccessFully!.",'group_unique_id'=>$group_res[0]->group_unique_id]);
                        }else{
                            return response()->json(['status'=>201,'message'=>"Failed!.",'group_id'=>null,'group_unique_id'=>null]);
                        }  
                    }else{
                        GroupLive::where('group_unique_id',$group_res[0]->group_unique_id)->update(['status'=>2]);
                        $user= new GroupLive;
                        $user->player_id=$request->player_id;
                        $user->group_id=$request->ante_amount_id.$request->player_no_id;
                        $user->ante_amount_id=$request->ante_amount_id;
                        ...
                        $res= $user->save();
                        if($res > 0){
                            $ques_res=Question::select('id')->inRandomOrder()->take(100)->get();
                            if(count($ques_res)>0){
                                foreach($ques_res as $val){
                                    $question= new GroupQuestion;
                                    $question->question_id = $val->id;
                                    $question->group_unique_id = $group_res[0]->group_unique_id;
                                    $question->save();
                                }
                            }
                           $ante_amnt = AnteAmount::where('id',$request->ante_amount_id)->value('ante_amount');
                            ...
                            return response()->json(['status'=>201,'message'=>"Failed!.",'group_unique_id'=>null]); 
                        }
                    }
                }else{
                    return response()->json(['status'=>201,'message'=>"Player Numbers not Existed.",'group_unique_id'=>null]);
                } 
            }   
        }else{
            ...
            if($res > 0){
                ...
                $bonus->save();
                return response()->json(['status'=>200,'message'=>"Player Added in Group SuccessFully!.",'group_unique_id'=>$group_unique_id]);
            }else{
                return response()->json(['status'=>201,'message'=>"Failed!.",'group_unique_id'=>null]); 
            }
        }       
