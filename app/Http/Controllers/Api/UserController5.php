...
class UserController extends Controller
{
    ...
    public function add_player_ingroup(Request $request){
        ...
        if(count($group_res)>0){
            ...
            'status'=>1,'player_id'=>$request->player_id])
            ->get();
            if(count($group_res1)>0){
                return response()->json(['status'=>202,'message'=>"Player Already Added in This Group.",'group_unique_id'=>$group_res1[0]->group_unique_id,'answer_status'=>$group_res1[0]->answer_status]);  
            }else{
                ...
                if($tl_players != '' && $tl_players != null){
                    if(count($group_res)+1 < $tl_players->player_number){
                        ...
                        if($res > 0){
                            ...
                            return response()->json(['status'=>200,'message'=>"Player Added in Group SuccessFully!.",'group_unique_id'=>$group_res[0]->group_unique_id,'answer_status'=>0]);
                        }else{
